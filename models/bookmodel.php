...
include "/classes/OpenItIreland/menubar.php";
...
class Bookmodel extends Model {
    ...
    function getBook(){
        ...
        $this->database->query($query);
        $book	=	$this->database->loadObject();
        $book->authors = $this->getAuthorNames($id);
        $book->isFavourite = $this->getFavourite($id);
    	return $book;
    }
    function getBook_1($id){
        $query = "SELECT * FROM publications WHERE id = '" .$id."'";	
  ...
  */  		
	...
	function addFavourite($fav)	{
		if((isset($_SESSION['id']) && (!$_SESSION['id']	=='') )) {
			$sql ="INSERT INTO favourites (userid, bookid) values(".$_SESSION['id'].', '.$fav.")";	
			$this->database->query($sql);
		}
	}
	function removeFavourite()	{
		if((isset($_SESSION['id']) && (!$_SESSION['id']	=='') )) {
			if(isset($_GET['Book_unfav'])) {
				$sql ="DELETE FROM favourites WHERE userid = '".$_SESSION['id']."' AND  bookid = '".$_GET['Book_unfav']."'";
			}
			else {
